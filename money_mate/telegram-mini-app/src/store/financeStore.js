...
const getMessage = (error) => error instanceof Error ? error.message : 'Неизвестная ошибка';
export const useFinanceStore = create()((set, get) => ({
    ...
    deleteOperation: async (row) => {
        ...
        try {
            ...
            set({
                mutationPending: false,
                operations: get().operations.filter((op) => row.id ? op.id !== row.id : op.value !== row.value || op.where !== row.where)
            });
        }
        catch (error) {
            console.error(error);
    ...
    deleteWish: async (row) => {
        ...
        try {
            ...
            set({
                mutationPending: false,
                wishes: get().wishes.filter((wish) => row.id ? wish.id !== row.id : wish.value !== row.value || wish.what !== row.what)
            });
        }
        catch (error) {
            console.error(error);
